...
Pdes = -36:1.0:-16; %Power Applied to PA (dBm)
...
%% Prepare Instruments
NRP18S = NRP_Setup() %Power Meter (input side)
NRP18SN = NRP_Setup18SN() %Power Meter (output side)
HP437B = HP437B_Setup(9, 13); %Powerr Meter (output end)

drain_supply = E36232A_Setup('higher'); %Power supply (gate)
gate_supply = E36232A_Setup('bottom'); %Power supply (drain)
...
for i = 1:1:length(Pavs)
    ...
    fprintf(FSW,message) 
    pause(2) % wait a second
    message = sprintf('CONF:GEN:RFO:STAT ON;*WAI');
    fprintf(FSW,message)
